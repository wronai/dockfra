...
# ── steps.py — credentials / setup ──────────────────────────────────────────
_add('creds_shortcut_title',
     pl='## 🔑 Credentials (skrót)', en='## 🔑 Credentials (shortcut)', de='## 🔑 Anmeldedaten (Kurzform)', fr='## 🔑 Identifiants (raccourci)',
     es='## 🔑 Credenciales (atajo)', it='## 🔑 Credenziali (scorciatoia)', pt='## 🔑 Credenciais (atalho)', cs='## 🔑 Přihlašovací údaje (zkratka)',
     ro='## 🔑 Credențiale (scurtătură)', nl='## 🔑 Inloggegevens (snelkoppeling)')
...
_add('ssh_key_path',
     pl='Ścieżka klucza SSH', en='SSH key path', de='SSH-Schlüsselpfad', fr='Chemin de la clé SSH',
     es='Ruta de clave SSH', it='Percorso chiave SSH', pt='Caminho da chave SSH', cs='Cesta k SSH klíči',
     ro='Calea cheii SSH', nl='SSH-sleutelpad')
_add('no_key_short',
     pl='(brak)', en='(none)', de='(keine)', fr='(aucune)',
     es='(ninguna)', it='(nessuna)', pt='(nenhuma)', cs='(žádný)',
     ro='(niciunul)', nl='(geen)')

# ── steps.py — launch errors ────────────────────────────────────────────────
_add('port_busy',
...
_add('show_port_blocker',
     pl='🔍 Pokaż co blokuje port {port}', en='🔍 Show what blocks port {port}', de='🔍 Zeige was Port {port} blockiert',
     fr='🔍 Montrer ce qui bloque le port {port}', es='🔍 Mostrar qué bloquea el puerto {port}',
     it='🔍 Mostra cosa blocca la porta {port}', pt='🔍 Mostrar o que bloqueia a porta {port}',
     cs='🔍 Ukázat co blokuje port {port}', ro='🔍 Arată ce blochează portul {port}', nl='🔍 Toon wat poort {port} blokkeert')
_add('auto_vnc_port',
     pl='🔧 Auto: użyj portu 6082 dla VNC', en='🔧 Auto: use port 6082 for VNC', de='🔧 Auto: Port 6082 für VNC verwenden',
     fr='🔧 Auto : utiliser le port 6082 pour VNC', es='🔧 Auto: usar puerto 6082 para VNC',
     it='🔧 Auto: usa porta 6082 per VNC', pt='🔧 Auto: usar porta 6082 para VNC',
     cs='🔧 Auto: použít port 6082 pro VNC', ro='🔧 Auto: folosește portul 6082 pentru VNC', nl='🔧 Auto: poort 6082 voor VNC gebruiken')
_add('change_port_retry',
     pl='🔄 Zmień port i spróbuj ponownie', en='🔄 Change port & retry', de='🔄 Port ändern & erneut versuchen',
     fr='🔄 Changer de port et réessayer', es='🔄 Cambiar puerto y reintentar',
     it='🔄 Cambia porta e riprova', pt='🔄 Mudar porta e tentar novamente',
     cs='🔄 Změnit port a zkusit znovu', ro='🔄 Schimbă portul și reîncearcă', nl='🔄 Poort wijzigen & opnieuw proberen')
...
_add('remove_network',
     pl='🔧 Usuń sieć `{net}`', en='🔧 Remove network `{net}`', de='🔧 Netzwerk `{net}` entfernen',
     fr='🔧 Supprimer le réseau `{net}`', es='🔧 Eliminar red `{net}`',
     it='🔧 Rimuovi rete `{net}`', pt='🔧 Remover rede `{net}`',
     cs='🔧 Odstranit síť `{net}`', ro='🔧 Șterge rețeaua `{net}`', nl='🔧 Netwerk `{net}` verwijderen')
_add('clean_unused_networks',
     pl='🔧 Wyczyść nieużywane sieci', en='🔧 Clean unused networks', de='🔧 Ungenutzte Netzwerke bereinigen',
     fr='🔧 Nettoyer les réseaux inutilisés', es='🔧 Limpiar redes no utilizadas',
     it='🔧 Pulisci reti inutilizzate', pt='🔧 Limpar redes não utilizadas',
     cs='🔧 Vyčistit nepoužívané sítě', ro='🔧 Curăță rețelele nefolosite', nl='🔧 Ongebruikte netwerken opschonen')
...
_add('auto_fix_compose',
     pl='🔧 Auto-napraw compose', en='🔧 Auto-fix compose', de='🔧 Compose auto-reparieren',
     fr='🔧 Auto-réparer compose', es='🔧 Auto-reparar compose',
     it='🔧 Auto-ripara compose', pt='🔧 Auto-corrigir compose',
     cs='🔧 Auto-opravit compose', ro='🔧 Auto-repară compose', nl='🔧 Compose auto-repareren')
...
_add('configure_creds',
     pl='🔑 Skonfiguruj credentials', en='🔑 Configure credentials', de='🔑 Anmeldedaten konfigurieren',
     fr='🔑 Configurer les identifiants', es='🔑 Configurar credenciales',
     it='🔑 Configura credenziali', pt='🔑 Configurar credenciais',
     cs='🔑 Konfigurovat přihlašovací údaje', ro='🔑 Configurează credențialele', nl='🔑 Inloggegevens configureren')
_add('show_missing_vars',
     pl='📄 Pokaż brakujące zmienne', en='📄 Show missing variables', de='📄 Fehlende Variablen anzeigen',
     fr='📄 Afficher les variables manquantes', es='📄 Mostrar variables faltantes',
     it='📄 Mostra variabili mancanti', pt='📄 Mostrar variáveis em falta',
     cs='📄 Zobrazit chybějící proměnné', ro='📄 Arată variabilele lipsă', nl='📄 Ontbrekende variabelen tonen')
...
_add('fix_docker_perms',
     pl='🔧 Napraw uprawnienia Docker', en='🔧 Fix Docker permissions', de='🔧 Docker-Berechtigungen reparieren',
     fr='🔧 Réparer les permissions Docker', es='🔧 Reparar permisos de Docker',
     it='🔧 Ripara permessi Docker', pt='🔧 Corrigir permissões Docker',
     cs='🔧 Opravit oprávnění Docker', ro='🔧 Repară permisiunile Docker', nl='🔧 Docker-rechten repareren')
_add('build_ssh_base_retry',
     pl='🔨 Zbuduj ssh-base i uruchom ponownie', en='🔨 Build ssh-base & retry', de='🔨 ssh-base bauen & erneut versuchen',
     fr='🔨 Construire ssh-base & réessayer', es='🔨 Construir ssh-base & reintentar',
     it='🔨 Costruisci ssh-base & riprova', pt='🔨 Construir ssh-base & tentar novamente',
     cs='🔨 Sestavit ssh-base & zkusit znovu', ro='🔨 Construiește ssh-base & reîncearcă', nl='🔨 ssh-base bouwen & opnieuw proberen')
_add('show_full_logs',
     pl='📋 Pokaż pełne logi', en='📋 Show full logs', de='📋 Vollständige Logs anzeigen',
     fr='📋 Afficher les journaux complets', es='📋 Mostrar registros completos',
     it='📋 Mostra log completi', pt='📋 Mostrar registos completos',
     cs='📋 Zobrazit plné logy', ro='📋 Arată jurnalele complete', nl='📋 Volledige logs tonen')
_add('stack_failed',
...
_add('change_git_url',
     pl='⚙️ Zmień GIT_REPO_URL', en='⚙️ Change GIT_REPO_URL', de='⚙️ GIT_REPO_URL ändern',
     fr='⚙️ Changer GIT_REPO_URL', es='⚙️ Cambiar GIT_REPO_URL',
     it='⚙️ Cambia GIT_REPO_URL', pt='⚙️ Alterar GIT_REPO_URL',
     cs='⚙️ Změnit GIT_REPO_URL', ro='⚙️ Schimbă GIT_REPO_URL', nl='⚙️ GIT_REPO_URL wijzigen')
_add('set_git_url',
     pl='⚙️ Ustaw GIT_REPO_URL', en='⚙️ Set GIT_REPO_URL', de='⚙️ GIT_REPO_URL setzen',
     fr='⚙️ Définir GIT_REPO_URL', es='⚙️ Establecer GIT_REPO_URL',
     it='⚙️ Imposta GIT_REPO_URL', pt='⚙️ Definir GIT_REPO_URL',
     cs='⚙️ Nastavit GIT_REPO_URL', ro='⚙️ Setează GIT_REPO_URL', nl='⚙️ GIT_REPO_URL instellen')
...
_add('cloned_to',
     pl='✅ Sklonowano do `{dir}`', en='✅ Cloned to `{dir}`', de='✅ Nach `{dir}` geklont',
     fr='✅ Cloné dans `{dir}`', es='✅ Clonado en `{dir}`',
     it='✅ Clonato in `{dir}`', pt='✅ Clonado para `{dir}`',
     cs='✅ Naklonováno do `{dir}`', ro='✅ Clonat în `{dir}`', nl='✅ Gekloond naar `{dir}`')
_add('app_no_folder',
...
_add('setup_github_llm',
     pl='🔑 Setup GitHub + LLM', en='🔑 Setup GitHub + LLM', de='🔑 GitHub + LLM einrichten',
     fr='🔑 Configurer GitHub + LLM', es='🔑 Configurar GitHub + LLM',
     it='🔑 Configura GitHub + LLM', pt='🔑 Configurar GitHub + LLM',
     cs='🔑 Nastavit GitHub + LLM', ro='🔑 Configurare GitHub + LLM', nl='🔑 GitHub + LLM instellen')
_add('task_integrations',
     pl='🔗 Integracje zadań', en='🔗 Task integrations', de='🔗 Aufgabenintegrationen',
     fr='🔗 Intégrations de tâches', es='🔗 Integraciones de tareas',
     it='🔗 Integrazioni attività', pt='🔗 Integrações de tarefas',
     cs='🔗 Integrace úkolů', ro='🔗 Integrări sarcini', nl='🔗 Taakintegraties')
_add('add_key_to_device',
     pl='Dodaj klucz do urządzenia:', en='Add key to device:', de='Schlüssel zum Gerät hinzufügen:',
     fr='Ajouter la clé à l\'appareil :', es='Agregar clave al dispositivo:',
     it='Aggiungi chiave al dispositivo:', pt='Adicionar chave ao dispositivo:',
     cs='Přidat klíč k zařízení:', ro='Adăugați cheia la dispozitiv:', nl='Sleutel aan apparaat toevoegen:')
_add('deploy_to',
     pl='## 🚀 Wdrożenie → `{target}`', en='## 🚀 Deploy → `{target}`', de='## 🚀 Bereitstellung → `{target}`',
     fr='## 🚀 Déploiement → `{target}`', es='## 🚀 Despliegue → `{target}`',
     it='## 🚀 Distribuzione → `{target}`', pt='## 🚀 Implementação → `{target}`',
     cs='## 🚀 Nasazení → `{target}`', ro='## 🚀 Implementare → `{target}`', nl='## 🚀 Deployen → `{target}`')
_add('run_config',
     pl='✅ Uruchom konfigurację', en='✅ Run configuration', de='✅ Konfiguration ausführen',
     fr='✅ Exécuter la configuration', es='✅ Ejecutar configuración',
     it='✅ Esegui configurazione', pt='✅ Executar configuração',
     cs='✅ Spustit konfiguraci', ro='✅ Rulează configurarea', nl='✅ Configuratie uitvoeren')
_add('change_creds',
     pl='✏️ Zmień credentials', en='✏️ Change credentials', de='✏️ Anmeldedaten ändern',
     fr='✏️ Modifier les identifiants', es='✏️ Cambiar credenciales',
     it='✏️ Modifica credenziali', pt='✏️ Alterar credenciais',
     cs='✏️ Změnit přihlašovací údaje', ro='✏️ Schimbă credențialele', nl='✏️ Inloggegevens wijzigen')
_add('config_done',
     pl='\n✅ Konfiguracja zakończona!', en='\n✅ Configuration done!', de='\n✅ Konfiguration abgeschlossen!',
     fr='\n✅ Configuration terminée !', es='\n✅ ¡Configuración finalizada!',
     it='\n✅ Configurazione completata!', pt='\n✅ Configuração concluída!',
     cs='\n✅ Konfigurace dokončena!', ro='\n✅ Configurare finalizată!', nl='\n✅ Configuratie voltooid!')
_add('configuring_github_llm',
     pl='⚙️ Konfiguruję GitHub + LLM...', en='⚙️ Configuring GitHub + LLM...', de='⚙️ Konfiguriere GitHub + LLM...',
     fr='⚙️ Configuration de GitHub + LLM...', es='⚙️ Configurando GitHub + LLM...',
     it='⚙️ Configurazione GitHub + LLM...', pt='⚙️ A configurar GitHub + LLM...',
     cs='⚙️ Konfiguruji GitHub + LLM...', ro='⚙️ Configurare GitHub + LLM...', nl='⚙️ GitHub + LLM configureren...')
_add('devices_launched',
     pl='✅ Devices stack uruchomiony!', en='✅ Devices stack launched!', de='✅ Devices-Stack gestartet!',
     fr='✅ Stack devices lancé !', es='✅ ¡Stack devices lanzado!',
     it='✅ Stack devices avviato!', pt='✅ Stack devices lançado!',
     cs='✅ Stack devices spuštěn!', ro='✅ Stack devices lansat!', nl='✅ Devices-stack gestart!')

# ── app.py — manager / suggest / engine ──────────────────────────────────────
_add('suggest_more',
     pl='🤖 Zaproponuj więcej', en='🤖 Suggest more', de='🤖 Mehr vorschlagen',
     fr='🤖 Suggérer plus', es='🤖 Sugerir más',
     it='🤖 Suggerisci altro', pt='🤖 Sugerir mais',
     cs='🤖 Navrhnout více', ro='🤖 Sugerează mai mult', nl='🤖 Meer voorstellen')
_add('force_retry',
     pl='🔄 Wymuś ponowienie', en='🔄 Force retry', de='🔄 Wiederholung erzwingen',
     fr='🔄 Forcer la réessai', es='🔄 Forzar reintento',
     it='🔄 Forza ripetizione', pt='🔄 Forçar nova tentativa',
     cs='🔄 Vynutit opakování', ro='🔄 Forțează reîncercarea', nl='🔄 Opnieuw afdwingen')
_add('retry_pipeline_adaptive',
     pl='🔄 Ponów pipeline (adaptacyjnie)', en='🔄 Retry pipeline (adaptive)', de='🔄 Pipeline wiederholen (adaptiv)',
     fr='🔄 Relancer le pipeline (adaptatif)', es='🔄 Reintentar pipeline (adaptivo)',
     it='🔄 Ripeti pipeline (adattivo)', pt='🔄 Repetir pipeline (adaptativo)',
     cs='🔄 Opakovat pipeline (adaptivně)', ro='🔄 Reîncercă pipeline (adaptiv)', nl='🔄 Pipeline opnieuw (adaptief)')
_add('show_logs_btn',
     pl='📋 Pokaż logi', en='📋 Show logs', de='📋 Logs anzeigen',
     fr='📋 Afficher les journaux', es='📋 Mostrar registros',
     it='📋 Mostra log', pt='📋 Mostrar registos',
     cs='📋 Zobrazit logy', ro='📋 Arată jurnalele', nl='📋 Logs tonen')
_add('full_ai_analysis',
     pl='🧠 Pełna analiza AI', en='🧠 Full AI analysis', de='🧠 Vollständige AI-Analyse',
     fr='🧠 Analyse IA complète', es='🧠 Análisis IA completo',
     it='🧠 Analisi AI completa', pt='🔧 Análise IA completa',
     cs='🧠 Plná analýza AI', ro='🧠 Analiză AI completă', nl='🧠 Volledige AI-analyse')

# ── discover.py ──────────────────────────────────────────────────────────────
_add('back_to_actions',
     pl='🔧 Wróć do akcji', en='🔧 Back to actions', de='🔧 Zurück zu Aktionen',
     fr='🔧 Retour aux actions', es='🔧 Volver a acciones',
     it='🔧 Torna alle azioni', pt='🔧 Voltar às ações',
     cs='🔧 Zpět k akcím', ro='🔧 Înapoi la acțiuni', nl='🔧 Terug naar acties')

# ── core.py — _CORE_ENV_SCHEMA labels ───────────────────────────────────────
...
_add('env_label_stacks',
     pl='Stacki do uruchomienia', en='Stacks to launch', de='Zu startende Stacks', fr='Stacks à lancer',
     es='Stacks a lanzar', it='Stack da avviare', pt='Stacks a lançar', cs='Stacky ke spuštění',
     ro='Stack-uri de lansat', nl='Te starten stacks')
_add('env_option_all',
     pl='Wszystkie', en='All', de='Alle', fr='Tous',
     es='Todos', it='Tutti', pt='Todos', cs='Všechny',
     ro='Toate', nl='Alle')
_add('custom_model_option',
     pl='✏️ Wpisz ręcznie…', en='✏️ Enter manually…', de='✏️ Manuell eingeben…', fr='✏️ Saisir manuellement…',
     es='✏️ Introducir manualmente…', it='✏️ Inserisci manualmente…', pt='✏️ Introduzir manualmente…', cs='✏️ Zadat ručně…',
     ro='✏️ Introduceți manual…', nl='✏️ Handmatig invoeren…')

# ── core.py — _HEALTH_PATTERNS messages ──────────────────────────────────────
_add('hp_port_conflict',
     pl='Konflikt portu — inny proces zajmuje port', en='Port conflict — another process is using the port',
     de='Portkonflikt — ein anderer Prozess nutzt den Port', fr='Conflit de port — un autre processus utilise le port',
     es='Conflicto de puerto — otro proceso usa el puerto', it='Conflitto porta — un altro processo usa la porta',
     pt='Conflito de porta — outro processo usa a porta', cs='Konflikt portu — jiný proces používá port',
     ro='Conflict de port — un alt proces folosește portul', nl='Poortconflict — een ander proces gebruikt de poort')
_add('hp_port_busy',
     pl='Port zajęty', en='Port busy', de='Port belegt', fr='Port occupé',
     es='Puerto ocupado', it='Porta occupata', pt='Porta ocupada', cs='Port obsazený',
     ro='Port ocupat', nl='Poort bezet')
_add('hp_no_perms',
...
_add('hp_missing_env',
     pl='Brakuje zmiennej środowiskowej', en='Missing environment variable',
     de='Fehlende Umgebungsvariable', fr='Variable d\'environnement manquante',
     es='Variable de entorno faltante', it='Variabile d\'ambiente mancante',
     pt='Variável de ambiente em falta', cs='Chybějící proměnná prostředí',
     ro='Variabilă de mediu lipsă', nl='Ontbrekende omgevingsvariabele')
_add('hp_no_network',
...
_add('hp_runtime_error',
     pl='Błąd Docker runtime', en='Docker runtime error', de='Docker-Laufzeitfehler',
     fr='Erreur runtime Docker', es='Error de runtime Docker',
     it='Errore runtime Docker', pt='Erro de runtime Docker', cs='Chyba Docker runtime',
     ro='Eroare runtime Docker', nl='Docker runtime-fout')
_add('hp_unhealthy',
...
_add('hp_addr_in_use',
     pl='Port zajęty przez inny proces', en='Port busy — used by another process',
     de='Port belegt — von einem anderen Prozess genutzt', fr='Port occupé par un autre processus',
     es='Puerto ocupado por otro proceso', it='Porta occupata da un altro processo',
     pt='Porta ocupada por outro processo', cs='Port obsazený jiným procesem',
     ro='Port ocupat de un alt proces', nl='Poort bezet door een ander proces')
...
# ── core.py — _HEALTH_PATTERNS button labels ────────────────────────────────
_add('diag_port_btn',
     pl='🔍 Diagnozuj port', en='🔍 Diagnose port', de='🔍 Port diagnostizieren',
     fr='🔍 Diagnostiquer le port', es='🔍 Diagnosticar puerto',
     it='🔍 Diagnostica porta', pt='🔍 Diagnosticar porta', cs='🔍 Diagnostikovat port',
     ro='🔍 Diagnosticați portul', nl='🔍 Poort diagnosticeren')
_add('change_port_btn',
     pl='⚙️ Zmień port', en='⚙️ Change port', de='⚙️ Port ändern',
     fr='⚙️ Changer de port', es='⚙️ Cambiar puerto',
     it='⚙️ Cambia porta', pt='⚙️ Alterar porta', cs='⚙️ Změnit port',
     ro='⚙️ Schimbă portul', nl='⚙️ Poort wijzigen')
_add('fix_perms_btn',
     pl='🔧 Napraw uprawnienia', en='🔧 Fix permissions', de='🔧 Berechtigungen reparieren',
     fr='🔧 Réparer les permissions', es='🔧 Reparar permisos',
     it='🔧 Ripara permessi', pt='🔧 Corrigir permissões', cs='🔧 Opravit oprávnění',
     ro='🔧 Repară permisiunile', nl='🔧 Rechten repareren')
_add('config_btn',
     pl='⚙️ Konfiguracja', en='⚙️ Configuration', de='⚙️ Konfiguration',
     fr='⚙️ Configuration', es='⚙️ Configuración',
     it='⚙️ Configurazione', pt='⚙️ Configuração', cs='⚙️ Konfigurace',
     ro='⚙️ Configurare', nl='⚙️ Configuratie')
_add('rebuild_btn',
     pl='🔧 Przebuduj', en='🔧 Rebuild', de='🔧 Neu erstellen',
     fr='🔧 Reconstruire', es='🔧 Reconstruir',
     it='🔧 Ricostruisci', pt='🔧 Reconstruir', cs='🔧 Přestavět',
     ro='🔧 Reconstruiește', nl='🔧 Herbouwen')
_add('fix_vol_perms_btn',
     pl='🔧 Napraw uprawnienia woluminu', en='🔧 Fix volume permissions', de='🔧 Volume-Berechtigungen reparieren',
     fr='🔧 Réparer les permissions du volume', es='🔧 Reparar permisos de volumen',
     it='🔧 Ripara permessi volume', pt='🔧 Corrigir permissões de volume', cs='🔧 Opravit oprávnění svazku',
     ro='🔧 Repară permisiunile volumului', nl='🔧 Volume-rechten repareren')
_add('fix_acme_btn',
     pl='🔧 Napraw ACME storage', en='🔧 Fix ACME storage', de='🔧 ACME-Speicher reparieren',
     fr='🔧 Réparer le stockage ACME', es='🔧 Reparar almacenamiento ACME',
     it='🔧 Ripara storage ACME', pt='🔧 Corrigir armazenamento ACME', cs='🔧 Opravit ACME úložiště',
     ro='🔧 Repară stocarea ACME', nl='🔧 ACME-opslag repareren')
_add('launch_all_btn',
     pl='🚀 Uruchom ponownie', en='🚀 Restart', de='🚀 Neu starten',
     fr='🚀 Redémarrer', es='🚀 Reiniciar',
     it='🚀 Riavvia', pt='🚀 Reiniciar', cs='🚀 Restartovat',
     ro='🚀 Repornește', nl='🚀 Herstarten')
_add('launch_all_full_btn',
     pl='🚀 Uruchom wszystko', en='🚀 Launch all', de='🚀 Alles starten',
     fr='🚀 Tout lancer', es='🚀 Lanzar todo',
     it='🚀 Avvia tutto', pt='🚀 Lançar tudo', cs='🚀 Spustit vše',
     ro='🚀 Lansează tot', nl='🚀 Alles starten')
_add('remove_conflicting_network',
     pl='🔧 Usuń konfliktującą sieć', en='🔧 Remove conflicting network', de='🔧 Konfliktierendes Netzwerk entfernen',
     fr='🔧 Supprimer le réseau en conflit', es='🔧 Eliminar red en conflicto',
     it='🔧 Rimuovi rete in conflitto', pt='🔧 Remover rede em conflito', cs='🔧 Odstranit konfliktní síť',
     ro='🔧 Șterge rețeaua conflictuală', nl='🔧 Conflicterend netwerk verwijderen')
_add('apply_fix_traefik',
     pl='✅ Zastosuj i napraw traefik', en='✅ Apply & fix traefik', de='✅ Anwenden & Traefik reparieren',
     fr='✅ Appliquer et réparer traefik', es='✅ Aplicar y reparar traefik',
     it='✅ Applica e ripara traefik', pt='✅ Aplicar e corrigir traefik', cs='✅ Aplikovat a opravit traefik',
     ro='✅ Aplică și repară traefik', nl='✅ Toepassen & traefik repareren')
_add('acme_storage_label',
     pl='Ścieżka ACME storage (traefik)', en='ACME storage path (traefik)', de='ACME-Speicherpfad (Traefik)',
     fr='Chemin de stockage ACME (traefik)', es='Ruta de almacenamiento ACME (traefik)',
     it='Percorso storage ACME (traefik)', pt='Caminho de armazenamento ACME (traefik)', cs='Cesta k ACME úložišti (traefik)',
     ro='Calea de stocare ACME (traefik)', nl='ACME-opslagpad (traefik)')
_add('acme_hint',
...
# ── core.py — _DIAGNOSIS_PATTERNS titles / labels ───────────────────────────
_add('diag_api_auth',
     pl='Błąd autoryzacji API', en='API authorization error', de='API-Autorisierungsfehler',
     fr='Erreur d\'autorisation API', es='Error de autorización API',
     it='Errore autorizzazione API', pt='Erro de autorização API', cs='Chyba autorizace API',
     ro='Eroare de autorizare API', nl='API-autorisatiefout')
...
_add('diag_no_github_token',
     pl='Brak tokenu GitHub', en='Missing GitHub token', de='Fehlendes GitHub-Token',
     fr='Token GitHub manquant', es='Token de GitHub faltante',
     it='Token GitHub mancante', pt='Token GitHub em falta', cs='Chybějící GitHub token',
     ro='Token GitHub lipsă', nl='Ontbrekend GitHub-token')
...
_add('diag_no_jira',
     pl='Brak konfiguracji Jira', en='Missing Jira configuration', de='Fehlende Jira-Konfiguration',
     fr='Configuration Jira manquante', es='Configuración de Jira faltante',
     it='Configurazione Jira mancante', pt='Configuração Jira em falta', cs='Chybějící konfigurace Jira',
     ro='Configurare Jira lipsă', nl='Ontbrekende Jira-configuratie')
...
_add('diag_ssh_error',
     pl='Błąd SSH / dostęp do repozytorium', en='SSH error / repository access',
     de='SSH-Fehler / Repository-Zugriff', fr='Erreur SSH / accès au dépôt',
     es='Error SSH / acceso al repositorio', it='Errore SSH / accesso al repository',
     pt='Erro SSH / acesso ao repositório', cs='Chyba SSH / přístup k repozitáři',
     ro='Eroare SSH / acces la depozit', nl='SSH-fout / toegang tot repository')
...
_add('diag_db_error',
     pl='Brak połączenia z bazą danych', en='Database connection failed',
     de='Datenbankverbindung fehlgeschlagen', fr='Connexion à la base de données échouée',
     es='Conexión a la base de datos fallida', it='Connessione al database fallita',
     pt='Ligação à base de dados falhou', cs='Připojení k databázi selhalo',
     ro='Conexiunea la baza de date a eșuat', nl='Databaseverbinding mislukt')
...
_add('db_host_label',
     pl='Host bazy danych', en='Database host', de='Datenbank-Host', fr='Hôte de base de données',
     es='Host de base de datos', it='Host database', pt='Host da base de dados', cs='Host databáze',
     ro='Host bază de date', nl='Database-host')
_add('db_password_label',
     pl='Hasło bazy danych', en='Database password', de='Datenbankpasswort', fr='Mot de passe base de données',
     es='Contraseña de base de datos', it='Password database', pt='Palavra-passe da base de dados', cs='Heslo databáze',
     ro='Parolă bază de date', nl='Databasewachtwoord')
...
_add('analyze_with_ai',
     pl='🧠 Analizuj z AI', en='🧠 Analyze with AI', de='🧠 Mit AI analysieren',
     fr='🧠 Analyser avec l\'IA', es='🧠 Analizar con IA',
     it='🧠 Analizza con AI', pt='🧠 Analisar com IA', cs='🧠 Analyzovat s AI',
     ro='🧠 Analizează cu AI', nl='🧠 Analyseren met AI')

# ── app.py — _step_pipeline_skip_implement ───────────────────────────────────
...
_add('local_tests',
     pl='### 🧪 Testy lokalne', en='### 🧪 Local tests', de='### 🧪 Lokale Tests',
     fr='### 🧪 Tests locaux', es='### 🧪 Pruebas locales',
     it='### 🧪 Test locali', pt='### 🧪 Testes locais', cs='### 🧪 Lokální testy',
     ro='### 🧪 Teste locale', nl='### 🧪 Lokale tests')
_add('commit_and_push',
     pl='### 📤 Commit & push', en='### 📤 Commit & push', de='### 📤 Commit & Push',
     fr='### 📤 Commit & push', es='### 📤 Commit & push',
     it='### 📤 Commit & push', pt='### 📤 Commit & push', cs='### 📤 Commit & push',
     ro='### 📤 Commit & push', nl='### 📤 Commit & push')
_add('nothing_to_commit',
     pl='ℹ️ Brak zmian do commitowania', en='ℹ️ Nothing to commit',
     de='ℹ️ Nichts zu committen', fr='ℹ️ Rien à commiter',
     es='ℹ️ Nada que commitear', it='ℹ️ Niente da committare',
     pt='ℹ️ Nada para commitar', cs='ℹ️ Nic k commitnutí',
     ro='ℹ️ Nimic de comis', nl='ℹ️ Niets om te committen')
_add('status_review',
     pl='### 📋 Status → review', en='### 📋 Status → review', de='### 📋 Status → Review',
     fr='### 📋 Statut → review', es='### 📋 Estado → review',
     it='### 📋 Stato → review', pt='### 📋 Estado → review', cs='### 📋 Status → review',
     ro='### 📋 Status → review', nl='### 📋 Status → review')
...
_add('retry_with_new_engine',
     pl='🔄 Ponów z nowym silnikiem', en='🔄 Retry with new engine', de='🔄 Mit neuer Engine erneut versuchen',
